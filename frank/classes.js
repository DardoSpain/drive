...
class Walker {
    constructor(x, y, id) {
      ...
      this.vel.mult(random(2, 5)); // Modulo aleatorio para la velocidad
    }
  
    // actualizar la posicion del walker
    update() {
      this.vel.rotate(map(noise(this.id, fc/50), 0,.94, -.1,.1));
